...
export default function MenuLateral({ children }) {
  ...
  const opçõesGerenteEmpresa = [
    { label: "Página Inicial", command: () => navegar("/pagina-inicial") },
    {
      label: "Menu", items: [
        {
          label: "Cadastrar Usuário", // (Seu label original)
          command: () => navegar("/atualizar-usuario"),
          disabled: usuárioLogado.status !== "ativo"
        },
        {
          label: "Cadastrar Gerente empresa", // (Seu label original)
          command: () => navegar("/cadastrar-gerente-empresa"),
        },

        // <-- ADICIONADO: O link que estava faltando
        {
          label: "Administrar Vagas Emprego",
          command: () => navegar("/administrar-vagas-emprego"),
          disabled: usuárioLogado.status !== "ativo"
        },
  ...
  const opçõesDiarista = [
    { label: "Página Inicial", command: () => navegar("/pagina-inicial") },
    {
      label: "Menu", items: [
        ...
        {
          label: "Cadastrar Diarista", // (Seu label original)
          command: () => navegar("/cadastrar-diarista")
        },

        // <-- ADICIONADO: Os links que estavam faltando
        {
